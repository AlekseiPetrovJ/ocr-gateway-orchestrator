...
> **Смотреть видео-демо (5 мин): [Java 21 + RabbitMQ + Docling v2 Offline Setup](https://youtu.be/5_nZiPtlTC4)**
...
- [x] **[ocr-worker/README.md](ocr-worker/README.md):** Оркестрация пайплайна (Cleanup -> OCR -> Docling). CPU/GPU ready
...
ВНИМАНИЕ: ОФЛАЙН И МОДЕЛИ
Библиотека Docling не качает модели при сборке образа (docker build), даже если прописать команду в Dockerfile. Она качает их только при реальном первом запуске парсинга.
Чтобы всё работало без интернета:

    Запусти воркер один раз с интернетом и прогони файлы.
    Скопируй папку /root/.cache из контейнера на ПК (docker cp).
```bash
docker cp $(docker compose ps -q ocr-worker-cpu):/root/.cache ./full_offline_cache
```
    В docker-compose примонтируй эту папку обратно в воркер.

Без этой папки на диске воркер в офлайне НЕ ЗАВЕДЕТСЯ.
